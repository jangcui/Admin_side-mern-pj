...
function Category({ params }: { params: { categoryId: string } }) {
    ...
    useEffect(() => {
        if (params.categoryId !== undefined) {
            dispatch(getProdCate(params.categoryId[0]));
        } else {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values: Omit<OptionType, 'id' | '_id'>) => {
            if (params.categoryId !== undefined) {
                await dispatch(updateAProdCate({ id: params.categoryId[0], title: formik.values.title }));
                formik.resetForm();
                dispatch(resetProdCateState());
            } else {
    ...
    return (
        <div className="wrapper">
            <h1 className="title"> {params.categoryId !== undefined ? 'Edit' : 'Add'} Product Category</h1>
            <form className="form" action="" onSubmit={formik.handleSubmit}>
                ...
                <button className="py-6 px-10 bg-primary rounded-2xl text-white mt-10" type={'submit'}>
                    {params.categoryId !== undefined ? 'Update' : 'Add'} Product Category
                </button>
            </form>
        </div>
    );
